...
import com.dexxiez.ohlauncher.helper.AppUsageStats
import com.dexxiez.ohlauncher.helper.AppUsageStatsBucket
import com.dexxiez.ohlauncher.helper.SingleLiveEvent
import com.dexxiez.ohlauncher.helper.WallpaperWorker
import com.dexxiez.ohlauncher.helper.convertEpochToMidnight
import com.dexxiez.ohlauncher.helper.formattedTimeSpent
import com.dexxiez.ohlauncher.helper.getAppsList
...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun getAppList(includeHiddenApps: Boolean = false) {
        viewModelScope.launch {
            appList.value =
                    getAppsList(appContext, prefs, includeRegularApps = true, includeHiddenApps)
        }
    }
    ...
    fun getTodaysScreenTime() {
        ...
        val usageStatsManager =
                appContext.getSystemService(USAGE_STATS_SERVICE) as UsageStatsManager
            ...
            if (events.getNextEvent(currentEvent)) {
                when (currentEvent.eventType) {
                    ...
                    UsageEvents.Event.FOREGROUND_SERVICE_STOP -> {
                        ...
                        if (packageEvents == null) packageEvents = ArrayList(listOf(currentEvent))
                        else packageEvents.add(currentEvent)
        ...
        for ((key, value) in eventsMap) {
            ...
            while (pos < value.size) {
                ...
                if (event.className != null) {
                    var backgroundBucket: AppUsageStatsBucket? =
                            backgroundBucketMap[event.className]
                    ...
                    when (event.eventType) {
                        UsageEvents.Event.ACTIVITY_RESUMED ->
                                foregroundBucket.startMillis = event.timeStamp
                        ...
                        UsageEvents.Event.FOREGROUND_SERVICE_START ->
                                backgroundBucket.startMillis = event.timeStamp
                        UsageEvents.Event.FOREGROUND_SERVICE_STOP ->
            ...
            val backgroundEnd: Long =
                    backgroundBucketMap.values.mapNotNull { it?.endMillis }.maxOrNull() ?: 0L

            val totalTimeBackground: Long =
                    backgroundBucketMap.values.mapNotNull { it?.totalTime }.sum()

            appUsageStatsHashMap[key] =
                    AppUsageStats(
